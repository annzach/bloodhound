...
/***/ function(module, exports, __webpack_require__) {
	...
	var Registration = _wrapComponent('_component')(_react3.default.createClass({
	  ...
	  getInitialState: function getInitialState() {
	    ...
	    try {
	      var lsPerson = JSON.parse(localStorage.localperson);
	    } catch (error) {
	      lsPerson = [];
	    }
	  ...
	  componentDidUpdate: function componentDidUpdate() {
	    localStorage.localperson = JSON.stringify(this.state.person);
	  },
	  onInputChange: function onInputChange(event) {
	    this.setState({ text: event.target.value });
	    console.log("changing Text...", event);
	  },
	  onSubmit: function onSubmit(e) {
	    e.preventDefault();
	  },
	  addDonor: function addDonor(text) {
	    var person = text.map(function (name) {
	      var newPerson = {};
	      newPerson.id = uuid();
	      newPerson.text = name;
	      return newPerson;
